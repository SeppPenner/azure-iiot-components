...
namespace Opc.Ua.Gds.Server.OpcVault {
    ...
    public class OpcVaultCertificateGroup : CertificateGroup {
        private readonly OpcVaultClientHandler _opcVaultHandler;
        ...
        public override async Task Init() {
            ...
            // erase old certs
            using (var store = CertificateStoreIdentifier.OpenStore(m_authoritiesStorePath)) {
                try {
                ...
                foreach (var rootCACertificate in rootCACertificateChain) {
                    var certs = await store.FindByThumbprint(rootCACertificate.Thumbprint);
                    if (certs.Count == 0) {
        ...
        protected async Task UpdateGroupStore(string storePath, X509Certificate2Collection certs, IList<Opc.Ua.X509CRL> crls) {
            if (!string.IsNullOrEmpty(storePath)) {
                using (var store = CertificateStoreIdentifier.OpenStore(storePath)) {
                    var oldCertificates = await store.Enumerate();
        ...
        public async Task UpdateIssuerCertificateAsync(X509Certificate2 signedCertificate) {
            var authority = FindAuthorityKeyIdentifier(signedCertificate);
            var subjectKeyId = FindSubjectKeyIdentifierExtension(Certificate);
